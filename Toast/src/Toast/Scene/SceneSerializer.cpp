...
namespace Toast {
	...
	static void SerializeEntity(YAML::Emitter& out, Entity entity, bool mainCamera = false)
	{
		...
		if (entity.HasComponent<SceneScriptComponent>())
		{
			out << YAML::Key << "SceneScriptComponent";
			out << YAML::BeginMap; // ScriptComponent
			...
			out << YAML::EndMap; // SceneScriptComponent
		}
	...
	void SceneSerializer::CopyComponents(Entity& target, Entity& source)
	{
		...
		CopyComponentIfExists<SceneScriptComponent>(target, target.GetScene()->mRegistry, source, source.GetScene()->mRegistry);
	...
	bool SceneSerializer::Deserialize(const std::string& filepath, Camera* editorCamera)
	{
		...
		if (entities) 
		{
			for (auto entity : entities)
			{
				...
				auto sceneScriptComponent = entity["SceneScriptComponent"];
				if (sceneScriptComponent)
				{
