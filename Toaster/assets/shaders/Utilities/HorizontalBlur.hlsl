...
float4 main(PixelInputType input) : SV_TARGET
{
    ...
    sum += baseTexture.Sample(clampSampler, input.texCoord + float2(-4.0f * texelSize.x, 0)) * 0.05f;
    sum += baseTexture.Sample(clampSampler, input.texCoord + float2(-3.0f * texelSize.x, 0)) * 0.09f;
    sum += baseTexture.Sample(clampSampler, input.texCoord + float2(-2.0f * texelSize.x, 0)) * 0.12f;
    sum += baseTexture.Sample(clampSampler, input.texCoord + float2(-1.0f * texelSize.x, 0)) * 0.15f;
