...
void TheNextFrontier2D::OnAttach()
{
	...
	Toast::FramebufferSpecification fbSpec;
	fbSpec.Width = 1280;
	fbSpec.Height = 720;
	mFramebuffer = Toast::Framebuffer::Create(fbSpec);
}
...
void TheNextFrontier2D::OnUpdate(Toast::Timestep ts)
{
	...
	Toast::Renderer2D::ResetStats();
	{
		...
		Toast::RenderCommand::SetRenderTargets(mFramebuffer);
...
void TheNextFrontier2D::OnImGuiRender()
{
	...
	if (dockingEnabled) 
	{
		...
		ImGui::Image(mFramebuffer->GetID(), ImVec2{ 1280.0f, 720.0f });
		ImGui::End();
